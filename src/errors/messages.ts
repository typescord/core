// From https://github.com/discordjs/discord.js/blob/master/src/errors/Messages.js
...
const $messages = {
	...
	// CLIENT_INVALID_OPTION: (property: string, must: string) => `The ${property} option must be ${must}`,
	...
	// BITFIELD_INVALID: (bit: string) => `Invalid bitfield flag or number: ${bit}.`,
	...
	// SHARDING_SHARD_NOT_FOUND: (id: number) => `Shard ${id} could not be found.`,
	...
	// SHARDING_WORKER_EXISTS: (id: number) => `Shard ${id} already has an active worker.`,
	...
	// SHARDING_NO_CHILD_EXISTS: (id: number) => `Shard ${id} has no active process or worker.`,
